...
def kpi_html_2026(k):
    ...
    return (
        ...
        f'<div class="kpi-val">{k["n"]}</div><div class="kpi-sub c-muted">Anio en curso</div></div>'
...
def build_html(logo_b64, data_js, all_kpis):
    ...
    for yr in YEARS:
        ...
        alert   = ""
        if yr == "2024":
            alert = '<div class="alert">Recurrentes 2024: margen real negativo. Ferrocarril Mexicano ($26.2M pendiente) es la exposicion critica.</div>'
        if yr == "2026":
            alert = '<div class="alert">AT&amp;T ($55.3M) sin estatus asignado. Costo devengado supera lo facturado — acelerar facturacion en Q2 es critico.</div>'
...
<style>
{HUB_NAV_CSS}
:root{{--green:#97D700;--dark:#1a1a1a;--surface:#242424;--border:rgba(125,194,66,0.15);--border-sub:rgba(255,255,255,0.07);--text:#f0f0f0;--text2:#9a9a9a;--text3:#666;--danger:#e05252;--warning:#d4962a;--info:#4a9edd;--r-sm:6px;--r-md:10px;--r-lg:16px;}}
*{{box-sizing:border-box;margin:0;padding:0;}}
body{{font-family:'Open Sans',sans-serif;background:var(--dark);color:var(--text);min-height:100vh;font-size:14px;line-height:1.6;}}
nav.port-nav{{display:flex;align-items:center;justify-content:space-between;padding:0 32px;height:56px;background:var(--dark);border-bottom:1px solid var(--border-sub);}}
.nav-meta{{font-size:12px;color:var(--text3);text-align:right;line-height:1.5;}}
.nav-meta span{{color:var(--green);font-weight:500;}}
...
.yr-tabs{{display:flex;gap:4px;background:var(--surface);padding:4px;border-radius:var(--r-md);border:1px solid var(--border-sub);}}
...
.yr-btn.active{{background:var(--green);color:#111;font-weight:600;}}
.kpi-strip{{display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:12px;margin-bottom:20px;}}
.kpi{{background:var(--surface);border:1px solid var(--border-sub);border-radius:var(--r-md);padding:14px 16px;}}
.kpi-lbl{{font-size:11px;color:var(--text3);text-transform:uppercase;letter-spacing:.06em;margin-bottom:6px;}}
.kpi-val{{font-size:21px;font-weight:600;letter-spacing:-.5px;}}
.kpi-sub{{font-size:11px;margin-top:4px;font-weight:500;}}
.c-good{{color:var(--green);}}.c-warn{{color:var(--warning);}}.c-bad{{color:var(--danger);}}.c-info{{color:var(--info);}}.c-muted{{color:var(--text3);}}
.prog{{height:3px;background:rgba(255,255,255,0.06);border-radius:2px;margin-top:6px;overflow:hidden;}}
.prog-fill{{height:100%;border-radius:2px;background:var(--green);}}
.alert{{background:rgba(224,82,82,0.1);border:1px solid rgba(224,82,82,0.3);border-radius:var(--r-md);padding:10px 16px;font-size:12.5px;color:#f08080;margin-bottom:14px;display:flex;align-items:flex-start;gap:8px;}}
.alert::before{{content:"!";flex-shrink:0;font-weight:700;margin-top:1px;}}
.seg{{background:var(--surface);border:1px solid var(--border-sub);border-radius:var(--r-lg);overflow:hidden;margin-bottom:10px;transition:border-color .2s;}}
.seg:hover{{border-color:var(--border);}}.seg.risk-high{{border-left:3px solid var(--danger);}}
.seg-hdr{{display:flex;align-items:center;justify-content:space-between;padding:11px 18px;background:rgba(255,255,255,0.025);border-bottom:1px solid var(--border-sub);}}
...
.b-gray{{background:rgba(255,255,255,0.08);color:var(--text3);}}.b-red{{background:rgba(224,82,82,0.18);color:#f08080;}}
.b-risk{{background:rgba(224,82,82,0.22);color:#ff6b6b;border:1px solid rgba(224,82,82,0.4);font-weight:700;}}
...
.seg-table th{{font-size:10.5px;font-weight:500;color:var(--text3);text-transform:uppercase;letter-spacing:.05em;padding:8px 18px;text-align:left;border-bottom:1px solid var(--border-sub);}}
.seg-table th.r{{text-align:right;}}
.seg-table td{{padding:9px 18px;font-size:13px;border-bottom:1px solid rgba(255,255,255,0.04);}}
.seg-table td.r{{text-align:right;font-variant-numeric:tabular-nums;font-size:12.5px;}}
.seg-table tr:last-child td{{border-bottom:none;}}.seg-table tr:hover td{{background:rgba(255,255,255,0.02);}}
...
.p-ok{{background:rgba(151,215,0,0.15);color:#97D700;}}.p-warn{{background:rgba(212,150,42,0.15);color:#d4962a;}}
.p-bad{{background:rgba(224,82,82,0.15);color:#f08080;}}.p-info{{background:rgba(74,158,221,0.15);color:#4a9edd;}}
.p-muted{{background:rgba(255,255,255,0.06);color:var(--text3);}}
.p-risk{{background:rgba(224,82,82,0.22);color:#ff6b6b;border:1px solid rgba(224,82,82,0.4);font-weight:700;}}
.risk-banner{{background:rgba(224,82,82,0.08);border-top:1px solid rgba(224,82,82,0.2);padding:10px 18px;font-size:12px;color:#f08080;display:flex;align-items:flex-start;gap:8px;}}
.drill-wrap{{display:none;border-top:1px solid var(--border-sub);padding:14px 18px;background:rgba(0,0,0,0.18);overflow-x:auto;}}
.drill-table{{width:100%;border-collapse:collapse;min-width:700px;}}
.drill-table th{{font-size:10px;font-weight:600;color:var(--text3);text-transform:uppercase;letter-spacing:.06em;padding:6px 8px;text-align:left;border-bottom:1px solid var(--border-sub);white-space:nowrap;}}
.drill-table th.r{{text-align:right;}}
.drill-table td{{padding:7px 8px;font-size:11.5px;border-bottom:1px solid rgba(255,255,255,0.03);vertical-align:top;color:var(--text2);}}
...
.drill-table tr:last-child td{{border-bottom:none;}}.drill-table tr:hover td{{background:rgba(255,255,255,0.02);}}
.yr-panel{{display:none;}}.yr-panel.active{{display:block;}}
footer{{border-top:1px solid var(--border-sub);padding:18px 32px;display:flex;align-items:center;justify-content:space-between;font-size:11px;color:var(--text3);}}
::-webkit-scrollbar{{width:5px;height:5px;}}::-webkit-scrollbar-track{{background:transparent;}}
::-webkit-scrollbar-thumb{{background:rgba(255,255,255,0.1);border-radius:3px;}}
::-webkit-scrollbar-thumb:hover{{background:rgba(151,215,0,0.4);}}
</style>
</head>
<body>
...
<nav class="port-nav">
  <div></div>
  <div class="nav-meta">Dashboard Ejecutivo &middot; Portafolio P&amp;S<br>Actualizado: <span>{CORTE_LABEL}</span> &middot; Elaborado por: Mtro. Alan Ceron C.</div>
</nav>
<main>
  <div class="page-header">
    <div>
      <div class="page-title">Portafolio de Proyectos y Servicios</div>
      <div class="page-sub">CEN Systems S.A. de C.V. &nbsp;&middot;&nbsp; Analisis financiero ano por ano &nbsp;&middot;&nbsp; Valores en MXN (millones)</div>
    </div>
    <div class="yr-tabs">
...
function semPill(s){{var map={{"OK >90d":"<span class=\\"pill p-ok\\">OK &gt;90d</span>","Alerta 31-60d":"<span class=\\"pill p-warn\\">Alerta 31-60d</span>","Critico \u226430d":"<span class=\\"pill p-bad\\">Critico \u226430d</span>","Vencido":"<span class=\\"pill p-bad\\">Vencido</span>","Vigilar 61-90d":"<span class=\\"pill p-info\\">Vigilar 61-90d</span>","Recurrente":"<span class=\\"pill p-muted\\">Facturacion parcial</span>","Riesgo Alto":"<span class=\\"pill p-risk\\">Riesgo Alto</span>"}};return map[s]||"<span class=\\"pill p-muted\\">"+s+"</span>";}}
